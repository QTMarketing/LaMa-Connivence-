...
interface DrinksClientProps {
  drinks: Drink[];
  featured: Drink[];
}

export default function DrinksClient({ drinks, featured }: DrinksClientProps) {
  const [selectedCategory, setSelectedCategory] = useState<'all' | Drink['category']>('all');
  const [searchQuery, setSearchQuery] = useState('');
  const { currentPromo, currentIndex, goToPromo, featuredDeals } = usePromo(featured);
