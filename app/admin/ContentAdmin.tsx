...
import { Save, Plus, Trash2, Edit2, X, Check, Eye, Star, AlertCircle, Info, LogOut, Search, Filter, Grid, List, TrendingUp, Tag, Calendar, MapPin, Store as StoreIcon, Phone, Clock, LayoutDashboard, FileText, Settings, Menu, X as XIcon, ShoppingBag, Percent, Briefcase } from 'lucide-react';
import { type Deal } from '@/lib/dealsData';
import { type Store } from '@/lib/storeData';
import { type Drink } from '@/lib/drinksData';
import { DEAL_CATEGORIES } from '@/lib/content/validation';
...
export type ContentTab = 'promos' | 'stores' | 'drinks';

const TAB_META: Record<ContentTab, { title: string; subtitle: string }> = {
  promos: {
    title: 'Promos & Deals',
    subtitle: 'Homepage and deals-page offers.',
  },
  drinks: {
    title: 'Drinks',
    subtitle: 'Drink deals and featured cold drinks.',
  },
  stores: {
    title: 'Store Locations',
    subtitle: 'Addresses, hours, and map pins.',
  },
};

export default function ContentAdmin({ tab }: { tab: ContentTab }) {
  ...
  const refreshDeals = useCallback(async () => {
    ...
    else if (res.status !== 403) setLoadError('Could not load deals.');
  }, []);

  const refreshStores = useCallback(async () => {
    const res = await fetch('/api/admin/stores');
    if (res.ok) setAllStores((await res.json()).stores);
    else if (res.status !== 403) setLoadError('Could not load stores.');
  }, []);

  const refreshDrinks = useCallback(async () => {
    const res = await fetch('/api/admin/drinks');
    if (res.ok) setAllDrinks((await res.json()).drinks);
    else if (res.status !== 403) setLoadError('Could not load drinks.');
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!session?.signedIn) {
        router.replace(`/admin/login?next=/admin/${tab === 'promos' ? 'deals' : tab}`);
        return;
      }

      setIsAuthenticated(true);
      // Only load the datasets this tab needs — keeps staff/owner requests lean.
      if (tab === 'promos') await refreshDeals();
      else if (tab === 'stores') await refreshStores();
      else await refreshDrinks();
  ...
  }, [router, refreshDeals, refreshStores, refreshDrinks, tab]);

  /** Sends a write and surfaces the server's message on failure. */
  ...
  const handleSave = async () => {
    ...
    const current = allDeals.find(d => d.id === editingId);
    if (!current) return;

    const merged = { ...current, ...editingDeal } as Deal;
    if (!await writeRequest(`/api/admin/deals/${editingId}`, 'PUT', merged)) return;
  ...
  const handleAdd = async () => {
    if (!newDeal.title || !newDeal.description || !newDeal.image || !newDeal.savings) {
      setSaveError('Title, description, image and savings are all required.');
      return;
    }

    // The id is assigned by Postgres, not guessed from the current list.
    if (!await writeRequest('/api/admin/deals', 'POST', newDeal)) return;
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!await writeRequest(`/api/admin/deals/${id}`, 'DELETE')) return;
    await refreshDeals();
  };
  ...
  const toggleFeatured = async (id: number) => {
    ...
    if (!deal) return;
    if (!await writeRequest(`/api/admin/deals/${id}`, 'PUT', { ...deal, featured: !deal.featured })) return;
    await refreshDeals();
  };
  ...
  const handleSaveStore = async () => {
    ...
    const current = allStores.find(s => s.id === editingStoreId);
    if (!current) return;

    const merged = { ...current, ...editingStore } as Store;
    if (!await writeRequest(`/api/admin/stores/${editingStoreId}`, 'PUT', merged)) return;
  ...
  const handleAddStore = async () => {
    if (!newStore.name || !newStore.address || !newStore.lat || !newStore.lng) {
      setSaveError('Name, address, latitude and longitude are all required.');
      return;
    }

    if (!await writeRequest('/api/admin/stores', 'POST', newStore)) return;
  ...
  const handleDeleteStore = async (id: number) => {
    ...
    if (!await writeRequest(`/api/admin/stores/${id}`, 'DELETE')) return;
    await refreshStores();
  };
  ...
  const handleSaveDrink = async () => {
    ...
    const current = allDrinks.find(d => d.id === editingDrinkId);
    if (!current) return;

    const merged = { ...current, ...editingDrink } as Drink;
    if (!await writeRequest(`/api/admin/drinks/${editingDrinkId}`, 'PUT', merged)) return;
  ...
  const handleAddDrink = async () => {
    ...
    if (!await writeRequest('/api/admin/drinks', 'POST', newDrink)) return;
  ...
  const handleDeleteDrink = async (id: number) => {
    ...
    if (!await writeRequest(`/api/admin/drinks/${id}`, 'DELETE')) return;
    await refreshDrinks();
  };

  const toggleDrinkFeatured = async (id: number) => {
    const drink = allDrinks.find(d => d.id === id);
    if (!drink) return;
    if (!await writeRequest(`/api/admin/drinks/${id}`, 'PUT', { ...drink, featured: !drink.featured })) return;
    await refreshDrinks();
  };
  ...
  // Every category the database accepts, so the dropdown, the type and the data
  // finally agree. The old list offered four values, three of which triggered
  // the homepage cache purge that deleted the promo you had just created.
  const categories: Array<{ value: Deal['category'] | 'all' | 'featured', label: string }> = [
    { value: 'all', label: 'All Promos' },
    { value: 'featured', label: 'Featured' },
    ...DEAL_CATEGORIES.map((value) => ({
      value,
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            {isAuthenticated ? 'Loading content...' : 'Checking authentication...'}
          </p>
        </div>
      </div>
    );
    ...
    <div>
        ...
        <header className="sticky top-0 z-30 border-b border-gray-200 bg-white shadow-sm">
          <div className="px-6 py-5">
            <h1 className="text-2xl font-black text-secondary">{meta.title}</h1>
