...
// ADAPTED FROM UPSTREAM mapcn.
// Upstream pointed MapLibre's worker at a CDN:
//   MapLibreGL.setWorkerUrl(`https://unpkg.com/maplibre-gl@.../maplibre-gl-worker.mjs`)
// Removed deliberately. Three reasons:
//   1. Our CSP allows `worker-src 'self' blob:` — a worker from unpkg.com is
//      refused, so the map rendered as a black canvas with no tiles.
//   2. maplibre-gl is a local dependency. Fetching its worker from a third
//      party at runtime is an avoidable supply-chain exposure on a commercial
//      site, and unpkg being down would take the store locator with it.
//   3. Without the override MapLibre bundles its own worker as a blob:, which
//      our CSP already permits.

// MapLibre needs an explicit worker URL. Upstream mapcn pointed this at
// unpkg.com; with that removed entirely, MapLibre requested a relative path
// that Next answers with a 404 HTML page, and the browser refused it:
//   "Failed to load module script: non-JavaScript MIME type text/html"
// So we SELF-HOST it. public/maplibre-gl-worker.mjs is copied verbatim from
// node_modules/maplibre-gl/dist/ — same origin, satisfies `worker-src 'self'`,
// no third-party runtime dependency, and it cannot break when a CDN does.
// If maplibre-gl is upgraded, re-copy that file to keep the versions in step.
